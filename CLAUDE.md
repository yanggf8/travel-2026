...
- **Naming caveat**: legacy artifacts may say `yokohama-travel-2026`, and much of this file still says "Japan" — the project started Japan-only and was EXTENDED to domestic Taiwan trips (first: `jiufen-2026`). Japan-specific machinery (OTA package shopping, JP entry rows, flight legs) simply doesn't apply to a self-drive domestic trip. (Root `package.json` is retired — see CLI Execution below.)
...
- **Accommodation is its own table family**, not `offers`: `domestic_accommodations` (+ child `domestic_accommodation_images` for the candidate gallery and `domestic_accommodation_ratings` for per-source guest ratings). These are slug-keyed GLOBAL reference data — **no `--plan-id`, no audit triad** (same family as `add-transit` / `add-omiyage`).
...
- **One row per review SOURCE** in `domestic_accommodation_ratings` — Booking.com scores out of 10 and Google out of 5, so the scale is stored with the score. Never average them into a single number; that publishes a rating nobody gave.
- **A published price needs its read date.** `price_source` + `price_checked_at` are rendered on the dashboard card; `update-accommodation --price-source` with no `--price-checked` stamps today. `validate data` WARNs once a rate is older than 21 days.
- Full command list: `docs/reference/CLI.md` -> "Domestic stay reference data".
...
- **No trip content in the migrator** — `db_migrate.rs` creates SCHEMA and registers destinations in `destination_config`; it must NEVER `INSERT` trip/reference content. A hardcoded three-row Jiufen accommodation seed used to live there, and because `db migrate` runs at the top of every integration test it RESURRECTED rows deleted through the CLI (the live dashboard kept showing a stay the DB no longer had; one of the three was a coffee shop wrongly recorded as a NT$7,200 sea-view room). Content enters only through a CLI command.
...
- **Destination config in DB** — `destination_config` table (no `data/destinations.json`); loaded from Turso. **130+ normalized tables** (see Data Model above). `flight_legs` holds fully-normalized flight data incl. `departure_terminal`/`arrival_terminal`.
...
- **Panic-safe + drift-proof teardown (REQUIRED).** These tests hit the SHARED live Turso DB, so a leaked test row pollutes production. Two rules, both enforced by `rust/crates/travel-cli/tests/common/mod.rs`:
  ...
  2. **Never assert on PRODUCTION rows, and make fixtures per-run unique.** Tests used to `INSERT OR IGNORE` fixtures whose ids collided with real trip rows (海論/山城逸境) and then assert on them, so CORRECTING A REAL PRICE turned the suite red — a data fix must not break tests. They also shared one fixture id across the tests in a file, and tests in a file run in PARALLEL, so the first to finish ran its `Guard` and deleted rows the others were still reading (the prod coupling had been hiding that race). Seed `zz_test_<nanos>_*` ids, keep fixture names short enough to survive the CLI table's 16-char `hotel_name` truncation, and have the `Guard` delete exactly that run's ids.
  3. **Arm the RAII `Guard` right after the plan-id is bound** — never call teardown as the last statement (a panicking assertion, i.e. every TDD RED run, unwinds past it and leaks). Pattern: `mod common; use common::Guard;` then `let _g = Guard::new({ let (plan,dest)=(plan.clone(),dest.clone()); move || common::teardown_plan(&plan,&dest) });`. Its `Drop` runs teardown on both return and panic. Keep an optional pre-clean `common::teardown_plan(...)` BEFORE the guard (clears a prior run's leftovers); never leave a trailing one. Proof/self-test: `tests/teardown_plan.rs` (seeds → teardown → asserts 0 rows across all plan-keyed tables).
...
├── workers/                       # trip-dashboard-rs (LIVE Rust worker), trip-dashboard-redirect (301), trip-dashboard-redirect
...
Dashboard plan-map split: `<plan-id>/plan.png` shows itinerary places with hotel and airport endpoints excluded from its bounds; `<plan-id>/plan-logistics.png` contains those hotel/airport endpoints. Per-day maps remain full route maps.
