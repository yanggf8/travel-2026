...
Dashboard plan-map split: `<plan-id>/plan.png` shows itinerary places with hotel and airport endpoints excluded from its bounds; `<plan-id>/plan-logistics.png` contains those hotel/airport endpoints. Per-day maps remain full route diagrams. The snapshot renderer draws stored OSM-derived coordinates and road geometry on a local grid and does not load OSM raster tiles.
