...
- **Worker `workers-rs` port — DONE & DEPLOYED** (PR #4, merged to master). The Rust dashboard lives at `workers/trip-dashboard-rs/` and is live at `trip-dashboard-rs.yanggf.workers.dev` (keyless maps, token auth, meal-pin links). The old TS worker (`workers/trip-dashboard/`) still exists and still serves the original URL; the `-rs` worker is meant to eventually reclaim it. **CUTOVER PREPARED (2026-07-02, deploy-gated — Yang runs wrangler):** `wrangler.toml` has an `[env.production]` block that ships `-rs` under `name = "trip-dashboard"` (`npx wrangler deploy --env production` reclaims the URL; default deploy still targets `-rs`). Runbook incl. the critical `PUBLIC_ORIGIN` secret + GitHub OAuth-callback update (else owner login breaks) + rollback: `docs/plans/2026-07-02-dashboard-cutover-ts-to-rs.md`. **Note:** cutover puts OAuth in front of the old URL (the TS worker was open); intended end-state (viewers still use `?token=` share links). Not yet executed. See "Trip Dashboard — two workers" below.
