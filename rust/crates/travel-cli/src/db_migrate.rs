...
const OTA_SOURCES: &[OtaSeed] = &[
    OtaSeed { source_id: "besttour", name: "喜鴻假期", types: &["package"], status: "active", scraper_script: Some("scripts/scrape_package.py"), regions: &["tokyo","kansai","hokkaido","kyushu","okinawa"], url_template: Some("https://www.besttour.com.tw"), notes: Some("Use listing URL for search, NOT /e_web/DOM/ (returns 404). Primarily group tours.") },
    ...
    OtaSeed { source_id: "tigerair", name: "台灣虎航", types: &["flight"], status: "active", scraper_script: Some("scripts/scrape_tigerair.py"), regions: &[], url_template: Some("https://www.tigerairtw.com"), notes: Some("Form-based scraper (no URL deep-linking), requires Playwright form interaction. No listing page.") },
    ...
    OtaSeed { source_id: "settour", name: "東南旅遊", types: &["package","flight","hotel"], status: "active", scraper_script: Some("scripts/scrape_package.py"), regions: &["tokyo","kansai","hokkaido","kyushu","okinawa"], url_template: Some("https://tour.settour.com.tw/search?destinationCode={dest_code}"), notes: Some("Group tour uses .product-item containers with code in slider-flightInfo_* IDs. FIT available at fit.settour.com.tw.") },
    OtaSeed { source_id: "travel4u", name: "山富旅遊", types: &["package"], status: "active", scraper_script: Some("scripts/scrape_package.py"), regions: &["tokyo","kansai","hokkaido","kyushu","okinawa","nagoya"], url_template: Some("https://www.travel4u.com.tw/group/area/{area_code}/japan/"), notes: Some("山富旅遊 - Taiwan OTA with group tours. Area codes map to Japan regions.") },
    OtaSeed { source_id: "eztravel", name: "易遊網", types: &["package","flight","hotel"], status: "active", scraper_script: Some("scripts/scrape_eztravel.py"), regions: &["tokyo","kansai","nagoya","okinawa","sapporo","fukuoka"], url_template: Some("https://packages.eztravel.com.tw/roundtrip-TPE-{dest_code}?checkin={depart_date}&checkout={return_date}&adult={pax}&child=0"), notes: Some("FIT at packages.eztravel.com.tw. Hotels shown may be in different city than destination (e.g., Kobe for Osaka). Check baggage - typically NOT included with LCC flights.") },
    ...
    OtaSeed { source_id: "agoda", name: "Agoda", types: &["hotel"], status: "active", scraper_script: Some("scripts/scrape_package.py"), regions: &[], url_template: Some("https://www.agoda.com/{hotel_slug}/hotel/{city_slug}-{country}.html?checkIn={checkin}&los={nights}&adults={adults}&rooms={rooms}&currency={currency}"), notes: Some("Direct hotel page URLs work reliably — full pricing, reviews, amenities. Search pages may return 0 results for dates >6 months out. Use hotel_page URL template for best results.") },
    ...
    OtaSeed { source_id: "google_flights", name: "Google Flights", types: &["flight"], status: "active", scraper_script: Some("scripts/scrape_package.py"), regions: &[], url_template: Some("https://www.google.com/travel/flights?q=Flights+to+{dest}+from+{origin}+on+{depart_date}+through+{return_date}&curr={currency}&hl=zh-TW"), notes: Some("Uses natural-language query URL format. Returns all-inclusive TWD prices with airline, times, duration, nonstop flags, and CO2 data. No form interaction needed.") },
];
...
async fn seed_ota_sources(conn: &libsql::Connection) {
    ...
    for s in OTA_SOURCES {
        ...
        exec_lenient(conn, &format!("DELETE FROM ota_source_regions WHERE source_id = {}", sq(s.source_id))).await;
        for r in s.regions {
