...
async fn bridge_audit_set_into_adopted_plan() {
    ...
    if db_exec("SELECT 1").is_none() {
        eprintln!("skipping tour-group-bridge test (no Turso creds)");
        return;
    }
    ...
    let subtypes = db_exec(&format!(
        "SELECT DISTINCT package_subtype FROM plan_offers WHERE plan_id = '{plan}'"
    ))
    .expect("creds");
    assert_eq!(subtypes.scalar().as_deref(), Some("group_tour"), "all bridged offers are group_tour");

    // plan_offer_group_meta: source_offer_run_id = run for each bridged offer.
    let meta_run = db_exec(&format!(
        "SELECT DISTINCT source_offer_run_id FROM plan_offer_group_meta WHERE plan_id = '{plan}'"
    ))
    .expect("creds");
    assert_eq!(meta_run.scalar().as_deref(), Some(run.as_str()), "group_meta carries source run_id");
    let meta_n = db_exec(&format!(
        "SELECT COUNT(*) FROM plan_offer_group_meta WHERE plan_id = '{plan}'"
    ))
    .expect("creds");
    assert_eq!(meta_n.scalar().as_deref(), Some("4"), "4 audit offers → 4 group_meta rows");
}