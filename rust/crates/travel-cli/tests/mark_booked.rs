...
fn seed(plan: &str, dest: &str, offer_id: &str, booking_key: &str) {
...
fn mark_booked_locks_full_write_surface() {
    ...
    if db_exec("SELECT 1 AS n").is_none() {
        eprintln!("skipping mark-booked test (no Turso creds)");
        return;
    }
    ...
    seed(&plan, &dest, &offer_id, &booking_key);
    ...
    let ver = db_exec(&format!("SELECT version AS v FROM plans WHERE plan_id = '{plan}'"))
        .expect("creds");
    assert_eq!(
        ver.scalar().as_deref(),
        Some("1"),
