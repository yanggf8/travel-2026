...
### funtime.com.tw — the FIT aggregator we should have started with

Discovered late in the June 2026 Okinawa research: **funtime.com.tw is a Taiwan travel-product aggregator that exposes a URL-controlled, server-rendered FIT-only search.** This is structurally the right tool for the methodology's FIT-comparable pillar — what we'd been trying to build ourselves by scraping each agency individually.

URL pattern:
```
https://www.funtime.com.tw/localtour/result.php
  ?from_city=TY          # 桃園/松山 (Taipei)
  &target=CT_OKINAWA     # destination region code
  &date_st=2026-06-10    # search window start
  &date_ed=2026-06-25    # search window end
  &type=fit              # ← key filter: FIT only, NOT group tours
```

**Why this changes the methodology:**

1. **One URL, many agencies.** funtime pulls actual bookable FIT products from KKday, BestTour, LionTravel, and others into a unified result page. Per-date cheapest is shown directly, with multiple options per date.
2. **Server-rendered.** Body innerText contains all titles, dates, and prices. No date-picker UI to drive, no async search wait.
...
| Date | Cheapest FIT TWD/人 | Options at date |
|------|---------------------|-----------------|
| 6/12 Fri | 11,900 | 8 |
| 6/17 Wed | 11,900 | 3 |
| **6/18 Thu** | **11,900** | 5 |
