...
./bin/travel derive-routes [--day N] [--dest slug] [--plan-id <id>]    # CASCADE: derive ai_recommended transit legs between consecutive same-day activities (from POI nearest_station + destination_transit metadata). Idempotent; skips days with a confirmed route; re-run --day N after activity edits. Run once after populate-itinerary. Legs flow into the 🤖 badge / validate-publish INFO / query-confirm lifecycle.
