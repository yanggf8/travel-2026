...
- destination reference (areas, POIs, clusters, transit, tips) — from Turso, via `./bin/travel query-destination-ref --slug {destination}`
...
1. **Read destination reference** — `./bin/travel query-destination-ref --slug {slug}` (reads Turso; throws if the destination is not registered)
